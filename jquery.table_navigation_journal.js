 ...
 */
jQuery.extendedNavigationJournal = {
	settings: null,
 ...
 */
jQuery.tableNavigationJournal = function(settings) {
	...
	if (jQuery.extendedNavigationJournal.settings == null) {
		jQuery.extendedNavigationJournal.settings = jQuery.extend({
			table_selector: 'table.navigateablejournal',
			row_selector: 'table.navigateablejournal tbody tr',
			selected_class: 'selectedjournal',
			activation_selector: 'a.activationjournal',
		...
		jQuery.extendedNavigationJournal.settings = jQuery.extend(jQuery.extendedNavigationJournal.settings, settings);
		jQuery.extendedNavigationJournal.tables = jQuery(jQuery.extendedNavigationJournal.settings.table_selector);
	 ...
	 * jQuery.extendedNavigationJournal.internal_event_hanlders object. This makes it possible to reuse the
	...
	function defineInternalEventHandlers() {
		jQuery.extendedNavigationJournal.internal_event_hanlders.click = function(){
			if ( jQuery.extendedNavigationJournal.settings.disabled ) { return }
			selectRow(this);
		};
		
		jQuery.extendedNavigationJournal.internal_event_hanlders.dblclick = function(){
			if ( jQuery.extendedNavigationJournal.settings.disabled ) { return }
			activateRow(this);
		};
		
		jQuery.extendedNavigationJournal.internal_event_hanlders.activation_link_click = function(){
			if ( jQuery.extendedNavigationJournal.settings.disabled ) { return }
			activateRow(jQuery(this).parents('tr').get(0));
			return false;
		};
		
		jQuery.extendedNavigationJournal.internal_event_hanlders.activation_link_focus = function(){
			if ( jQuery.extendedNavigationJournal.settings.disabled ) { return }
			selectRow(jQuery(this).parents('tr').get(0));
		};
		
		jQuery.extendedNavigationJournal.internal_event_hanlders.keypress = function(e){
			if ( jQuery.extendedNavigationJournal.settings.disabled ) { return }
			...
			switch (key_code){
				...
				case 39: // arrow right
					...
					if (next_row.length > 0) {
						...
						return r;
					} else if ( jQuery.extendedNavigationJournal.settings.jump_between_tables ) {
						var current_table = getSelectedRow().parents(jQuery.extendedNavigationJournal.settings.table_selector).get(0);
						var all_tables = jQuery.extendedNavigationJournal.tables;
	...
	function bindInternalEventHandlers() {
		var selectable_rows = jQuery(jQuery.extendedNavigationJournal.settings.row_selector);
		var selected_row = selectable_rows.filter("." + jQuery.extendedNavigationJournal.settings.selected_class);
		...
		selectable_rows.bind(jQuery.extendedNavigationJournal.settings.select_event, {}, jQuery.extendedNavigationJournal.internal_event_hanlders.click);
		selectable_rows.bind(jQuery.extendedNavigationJournal.settings.activate_event, {}, jQuery.extendedNavigationJournal.internal_event_hanlders.dblclick);
		
		var activation_element = selectable_rows.find(jQuery.extendedNavigationJournal.settings.activation_selector);
		if ( activation_element && jQuery.extendedNavigationJournal.settings.activation_element_activate_event ) {
			selectable_rows.find(jQuery.extendedNavigationJournal.settings.activation_selector).bind(jQuery.extendedNavigationJournal.settings.activation_element_activate_event, {}, jQuery.extendedNavigationJournal.internal_event_hanlders.activation_link_click);
		}
		activation_element.focus(jQuery.extendedNavigationJournal.internal_event_hanlders.activation_link_focus);
		
		if (jQuery.extendedNavigationJournal.settings.bind_key_events_to_links) {
			var bind_to = jQuery.extendedNavigationJournal.settings.row_selector + ' ' + jQuery.extendedNavigationJournal.settings.activation_selector;
		} else {
		...
		if (jQuery.browser.msie) {
			jQuery(bind_to).keydown(jQuery.extendedNavigationJournal.internal_event_hanlders.keypress);
		} else {
			jQuery(bind_to).keypress(jQuery.extendedNavigationJournal.internal_event_hanlders.keypress);
		}
	};
	 ...
	 * executed. See "jQuery.extendedNavigationJournal.settings.on_select" configuration option.
	...
	function selectRow(row) {
		if (row != undefined && row.nodeName.toLowerCase() == "tr"){
			var do_default_action = executeEventHandler(row, jQuery.extendedNavigationJournal.settings.on_select);
			
			if (do_default_action)
			{
				getSelectedRow().removeClass(jQuery.extendedNavigationJournal.settings.selected_class);
				jQuery(row).addClass(jQuery.extendedNavigationJournal.settings.selected_class);
				
				if (jQuery.extendedNavigationJournal.settings.focus_links_on_select) {
					var activation_element = jQuery(row).find(jQuery.extendedNavigationJournal.settings.activation_selector)[0];
					if (activation_element != null) {
						activation_element.focus();
					}
				}
				
				if (jQuery.extendedNavigationJournal.settings.focus_tables) {
					jQuery.extendedNavigationJournal.tables.removeClass(jQuery.extendedNavigationJournal.settings.focused_table_class);
					jQuery(row).parents(jQuery.extendedNavigationJournal.settings.table_selector).eq(0).addClass(jQuery.extendedNavigationJournal.settings.focused_table_class);
				}
	...
	function getSelectedRow() {
		return jQuery(jQuery.extendedNavigationJournal.settings.row_selector + "." + jQuery.extendedNavigationJournal.settings.selected_class);
	}
	 ...
	 *   "jQuery.extendedNavigationJournal.settings.cookie_name" is not null.
	 * - If present the user specified on_activate event handler will be
	 *   executed. See "jQuery.extendedNavigationJournal.settings.on_activate" or the
	 ...
	 *   element selected by the "jQuery.extendedNavigationJournal.settings.activation_selector"
	...
	function activateRow(row) {
		if (row != undefined) {
			...
			var activated_index = jQuery(jQuery.extendedNavigationJournal.settings.row_selector).index(row);
			storeLastRowIndex(activated_index);
			
			var do_default_action = executeEventHandler(row, jQuery.extendedNavigationJournal.settings.on_activate);
			
			if (do_default_action)
			{
				var activation_link = jQuery(jQuery.extendedNavigationJournal.settings.activation_selector, row);
	...
	function executeEventHandler(row, action) {
		...
		{
			try {
				var onclick_of_target_link = jQuery(jQuery.extendedNavigationJournal.settings.activation_selector, row).attr(action);
	...
	function scrollToIfNeccessary(e) {
		...
		var calculated_boundaries = {
			top: element_position.y - jQuery.extendedNavigationJournal.settings.scroll_overlap,
			bottom: element_position.y + element_position.hb + jQuery.extendedNavigationJournal.settings.scroll_overlap,
			left: element_position.x - jQuery.extendedNavigationJournal.settings.scroll_overlap,
			right: element_position.x + element_position.wb + jQuery.extendedNavigationJournal.settings.scroll_overlap
		};
	...
	function readLastRowIndex() {
		if (jQuery.extendedNavigationJournal.settings.cookie_name)
		{
			var data = readCookie(jQuery.extendedNavigationJournal.settings.cookie_name);
			if (data) {
	...
	function storeLastRowIndex(index) {
		if (jQuery.extendedNavigationJournal.settings.cookie_name)
		{
			...
			createCookie(jQuery.extendedNavigationJournal.settings.cookie_name, data);
 		}
	};
	...
	function eraseLastRowIndexCookie() {
		eraseCookie(jQuery.extendedNavigationJournal.settings.cookie_name);
	};
