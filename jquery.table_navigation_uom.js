 ...
 * objects and values which have to exist after or outside the jQuery.tableNavigationUom()
 * function (defined after this namespace).
 */
jQuery.extendedNavigationUom = {
	settings: null,
 ...
 */
jQuery.tableNavigationUom = function(settings) {
	...
	// However if the global settings are already defined the tableNavigationUom() function was already called and all
	...
	// multiple times from different tableNavigationUom instances (keypress event will be process x times and therefore
	...
	if (jQuery.extendedNavigationUom.settings == null) {
		jQuery.extendedNavigationUom.settings = jQuery.extend({
			table_selector: 'table.navigateableuom',
			row_selector: 'table.navigateableuom tbody tr',
			selected_class: 'selecteduom',
			activation_selector: 'a.activationuom',
		...
		jQuery.extendedNavigationUom.settings = jQuery.extend(jQuery.extendedNavigationUom.settings, settings);
		jQuery.extendedNavigationUom.tables = jQuery(jQuery.extendedNavigationUom.settings.table_selector);
	 ...
	 * jQuery.extendedNavigationUom.internal_event_hanlders object. This makes it possible to reuse the
	 * event handlers later on (eg. by the bindInternalEventHandlers() function when calling
	 * jQuery.tableNavigationUom() multiple times).
	 * 
	...
	function defineInternalEventHandlers() {
		jQuery.extendedNavigationUom.internal_event_hanlders.click = function(){
			if ( jQuery.extendedNavigationUom.settings.disabled ) { return }
			selectRow(this);
		};
		
		jQuery.extendedNavigationUom.internal_event_hanlders.dblclick = function(){
			if ( jQuery.extendedNavigationUom.settings.disabled ) { return }
			activateRow(this);
		};
		
		jQuery.extendedNavigationUom.internal_event_hanlders.activation_link_click = function(){
			if ( jQuery.extendedNavigationUom.settings.disabled ) { return }
			activateRow(jQuery(this).parents('tr').get(0));
			return false;
		};
		
		jQuery.extendedNavigationUom.internal_event_hanlders.activation_link_focus = function(){
			if ( jQuery.extendedNavigationUom.settings.disabled ) { return }
			selectRow(jQuery(this).parents('tr').get(0));
		};
		
		jQuery.extendedNavigationUom.internal_event_hanlders.keypress = function(e){
			//e.preventDefault();
			if ( jQuery.extendedNavigationUom.settings.disabled ) { return }
			...
			switch (key_code){
				...
				case 39: // arrow right
					...
					if (next_row.length > 0) {
						...
						return r;
					} else if ( jQuery.extendedNavigationUom.settings.jump_between_tables ) {
						var current_table = getSelectedRow().parents(jQuery.extendedNavigationUom.settings.table_selector).get(0);
						var all_tables = jQuery.extendedNavigationUom.tables;
	...
	function bindInternalEventHandlers() {
		var selectable_rows = jQuery(jQuery.extendedNavigationUom.settings.row_selector);
		var selected_row = selectable_rows.filter("." + jQuery.extendedNavigationUom.settings.selected_class);
		...
		selectable_rows.bind(jQuery.extendedNavigationUom.settings.select_event, {}, jQuery.extendedNavigationUom.internal_event_hanlders.click);
		selectable_rows.bind(jQuery.extendedNavigationUom.settings.activate_event, {}, jQuery.extendedNavigationUom.internal_event_hanlders.dblclick);
		
		var activation_element = selectable_rows.find(jQuery.extendedNavigationUom.settings.activation_selector);
		if ( activation_element && jQuery.extendedNavigationUom.settings.activation_element_activate_event ) {
			selectable_rows.find(jQuery.extendedNavigationUom.settings.activation_selector).bind(jQuery.extendedNavigationUom.settings.activation_element_activate_event, {}, jQuery.extendedNavigationUom.internal_event_hanlders.activation_link_click);
		}
		activation_element.focus(jQuery.extendedNavigationUom.internal_event_hanlders.activation_link_focus);
		
		if (jQuery.extendedNavigationUom.settings.bind_key_events_to_links) {
			var bind_to = jQuery.extendedNavigationUom.settings.row_selector + ' ' + jQuery.extendedNavigationUom.settings.activation_selector;
		} else {
		...
		if (jQuery.browser.msie) {
			jQuery(bind_to).keydown(jQuery.extendedNavigationUom.internal_event_hanlders.keypress);
		} else {
			jQuery(bind_to).keypress(jQuery.extendedNavigationUom.internal_event_hanlders.keypress);
		}
	};
	 ...
	 * executed. See "jQuery.extendedNavigationUom.settings.on_select" configuration option.
	...
	function selectRow(row) {
		if (row != undefined && row.nodeName.toLowerCase() == "tr"){
			var do_default_action = executeEventHandler(row, jQuery.extendedNavigationUom.settings.on_select);
			
			if (do_default_action)
			{
				getSelectedRow().removeClass(jQuery.extendedNavigationUom.settings.selected_class);
				jQuery(row).addClass(jQuery.extendedNavigationUom.settings.selected_class);
				
				if (jQuery.extendedNavigationUom.settings.focus_links_on_select) {
					var activation_element = jQuery(row).find(jQuery.extendedNavigationUom.settings.activation_selector)[0];
					if (activation_element != null) {
						activation_element.focus();
					}
				}
				
				if (jQuery.extendedNavigationUom.settings.focus_tables) {
					jQuery.extendedNavigationUom.tables.removeClass(jQuery.extendedNavigationUom.settings.focused_table_class);
					jQuery(row).parents(jQuery.extendedNavigationUom.settings.table_selector).eq(0).addClass(jQuery.extendedNavigationUom.settings.focused_table_class);
				}
	...
	function getSelectedRow() {
		return jQuery(jQuery.extendedNavigationUom.settings.row_selector + "." + jQuery.extendedNavigationUom.settings.selected_class);
	}
	 ...
	 *   "jQuery.extendedNavigationUom.settings.cookie_name" is not null.
	 * - If present the user specified on_activate event handler will be
	 *   executed. See "jQuery.extendedNavigationUom.settings.on_activate" or the
	 ...
	 *   element selected by the "jQuery.extendedNavigationUom.settings.activation_selector"
	...
	function activateRow(row) {
		if (row != undefined) {
			...
			var activated_index = jQuery(jQuery.extendedNavigationUom.settings.row_selector).index(row);
			storeLastRowIndex(activated_index);
			
			var do_default_action = executeEventHandler(row, jQuery.extendedNavigationUom.settings.on_activate);
			
			if (do_default_action)
			{
				var activation_link = jQuery(jQuery.extendedNavigationUom.settings.activation_selector, row);
	...
	function executeEventHandler(row, action) {
		...
		{
			try {
				var onclick_of_target_link = jQuery(jQuery.extendedNavigationUom.settings.activation_selector, row).attr(action);
	...
	function scrollToIfNeccessary(e) {
		...
		var calculated_boundaries = {
			top: element_position.y - jQuery.extendedNavigationUom.settings.scroll_overlap,
			bottom: element_position.y + element_position.hb + jQuery.extendedNavigationUom.settings.scroll_overlap,
			left: element_position.x - jQuery.extendedNavigationUom.settings.scroll_overlap,
			right: element_position.x + element_position.wb + jQuery.extendedNavigationUom.settings.scroll_overlap
		};
	...
	function readLastRowIndex() {
		if (jQuery.extendedNavigationUom.settings.cookie_name)
		{
			var data = readCookie(jQuery.extendedNavigationUom.settings.cookie_name);
			if (data) {
	...
	function storeLastRowIndex(index) {
		if (jQuery.extendedNavigationUom.settings.cookie_name)
		{
			...
			createCookie(jQuery.extendedNavigationUom.settings.cookie_name, data);
 		}
	};
	...
	function eraseLastRowIndexCookie() {
		eraseCookie(jQuery.extendedNavigationUom.settings.cookie_name);
	};
