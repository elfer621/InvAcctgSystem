...
if($_POST){
	...
	$receipt="insert into tbl_receipt_manual (`receipt`,`date`,`cust_id`,`receivedfrom`,`paymentof`,
		`cash_amt`,`check_amt`,`check_details`,`check_date`,`particular`,rent_amount,other_amount,total_amount) values 
		('{$_REQUEST['ornum']}','".date('Y-m-d')."','{$_REQUEST['CRJ']['customer_name']}','{$_REQUEST['CRJ']['receivedfrom']}','{$_REQUEST['CRJ']['paymentof']}',
		'{$_REQUEST['CRJ']['cash_amt']}','{$_REQUEST['CRJ']['check_amt']}','".$_REQUEST['CRJ']['check_bank']." ".$_REQUEST['CRJ']['check_num']."',
		'{$_REQUEST['CRJ']['check_date']}','".serialize($rec)."','".preg_replace("/[^0-9.]/", "", $_REQUEST['rent_amount'])."','".preg_replace("/[^0-9.]/", "", $_REQUEST['other_amount'])."','".preg_replace("/[^0-9.]/", "", $_REQUEST['xtotal'])."')";
...
<form method="post" name="frm_received_payment" id="frm_received_payment">
	<div style="float:left;width:45%;">
		...
		<input type="text" name="ornum" style="float:left;width:100px;margin-right:10px;"/>
		...
		<input readonly type="text" name="CRJ[receivedfrom]" style="float:left;width:250px;margin-right:10px;"/>
		<div style="clear:both;height:5px;"></div>
		<div style="float:left;margin-right:10px;width:100px;">Address</div>
		<input readonly type="text" name="CRJ[customer_address]" style="float:left;width:250px;margin-right:10px;"/>
		<div style="clear:both;height:5px;"></div>
		<div style="float:left;margin-right:10px;width:100px;">Business Style</div>
		<input readonly type="text" name="CRJ[business_style]" style="float:left;width:250px;margin-right:10px;"/>
		<div style="clear:both;height:5px;"></div>
		<div style="float:left;margin-right:10px;width:100px;">TIN#</div>
		<input readonly type="text" name="CRJ[customer_tin]" style="float:left;width:250px;margin-right:10px;"/>
		<div style="clear:both;height:5px;"></div>
		<div style="clear:both;height:5px;"></div>
		<div style="float:left;margin-right:10px;width:100px;">Payment of</div>
		<input readonly type="text" name="CRJ[paymentof]" style="float:left;width:250px;margin-right:10px;"/>
		<div style="clear:both;height:5px;"></div>
		<input type="button" style="height:30px;width:100px;" value="Save" onclick="saveForm();"/>
	</div>
			...
			<thead>
				<tr>
					...
					<td style="border:1px solid #1DA5EA;">
						<input type="text" id="rent_amount" name="rent_amount" style="text-align:right;background:transparent;border:none;width:100%;"/>
					</td>
				</tr>
				<tr>
					<td style="border:1px solid #1DA5EA;" width="600px">Other Charges</td>
					<td style="border:1px solid #1DA5EA;">
						<input type="text" id="other_amount" name="other_amount" style="text-align:right;background:transparent;border:none;width:100%;" readonly />
					</td>
				</tr>
				<tr>
					<td style="border:1px solid #1DA5EA;" width="600px">Total</td>
					<td style="border:1px solid #1DA5EA;">
						<input type="text" id="xtotal" name="xtotal" style="text-align:right;background:transparent;border:none;width:100%;" readonly />
					</td>
				</tr>
				<tr>
...
<fieldset>
	...
	<input onkeypress="return addParticular(event,this)" type="text" name="addparticular" id="addparticular" style="float:right;height:25px;width:80%"/>
</fieldset>
<div style="clear:both;height:10px;"></div>
<fieldset>
	<legend>Menu</legend>
	...
	<input id="bt9" class="buthov" type="button" onclick="viewReceiptManual()" value="Receipt Printing" style="height:40px;width:150px;float:right;"/>
</fieldset>
<script>
...
$("#rent_amount").change(function(){
	...
	$("#other_amount").val(new Number(total-rent).formatMoney(2));
...
function viewReceiptManual(){
	clickDialog('dialogbox',700,400,'receipt_manual','Receipt List');
}
