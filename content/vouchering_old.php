...
if($_POST){
	...
	if(!$qry1){
		echo "Saving Vouchering: ".mysql_error();
			...
			// $checkinfo_header = ",payto,check_date,check_number,bank,bank_validation";
			// $ar_refid_header = ",ar_refid";
			// $ap_refid_header = ",ap_refid";
			// $sql = "insert into tbl_journal_entry (id,refid,fiscal_year,type,`date`,account_code,account_desc,dr,cr,account_group,
				// account_type,report_type,subsidiary,cash_in_bank $checkinfo_header $ar_refid_header $ap_refid_header,center) values ";
			$flag=false;
			for($i=0;$i<count($_REQUEST['code']);$i++){
				/*
				//get cash in bank details
				$checkinfo_data = ",'{$_REQUEST['checkinfo'][$_REQUEST['code'][$i]]['payto']}','{$_REQUEST['checkinfo'][$_REQUEST['code'][$i]]['check_date']}','{$_REQUEST['checkinfo'][$_REQUEST['code'][$i]]['check_num']}','{$_REQUEST['checkinfo'][$_REQUEST['code'][$i]]['check_bank']}','{$_REQUEST['checkinfo'][$_REQUEST['code'][$i]]['bank_validation']}'";
				//get cash in bank details
				//ar ref
				$ar_refid_data=",'{$_REQUEST['ar_refid'][$_REQUEST['code'][$i]]}'";
				//ar ref
				//ap ref
				$ap_refid_data=",'{$_REQUEST['ap_refid'][$_REQUEST['code'][$i]]}'";
				//ap ref
				$account_info = $db->getWHERE("*","tbl_chart_of_account","where account_code='{$_REQUEST['code'][$i]}'");
				if($flag)$sql_data.=",";
				$sql_data.="('{$_REQUEST['id'][$i]}',$glref,'{$_REQUEST['fiscal_year']}','{$_REQUEST['type']}','{$_REQUEST['xdate']}','{$_REQUEST['code'][$i]}','{$_REQUEST['desc'][$i]}',
					'".preg_replace("/[^0-9.]/", "", $_REQUEST['dr_amt'][$i])."','".preg_replace("/[^0-9.]/", "", $_REQUEST['cr_amt'][$i])."','{$account_info['account_group']}','{$account_info['account_type']}','{$account_info['report_type']}','{$account_info['subsidiary']}',
					'{$account_info['cash_in_bank']}' $checkinfo_data $ar_refid_data $ap_refid_data,'{$_REQUEST['cost_center'][$_REQUEST['code'][$i]]}')";
				$flag=true;
				*/
				$account_info = $db->getWHERE("*","tbl_chart_of_account","where account_code='{$_REQUEST['code'][$i]}'");
				...
				$data=array(
					'id'=>$_REQUEST['id'][$i],
					'refid'=>$glref,
					'center'=>$_REQUEST['cost_center'][$_REQUEST['code'][$i]],
					'fiscal_year'=>($_REQUEST['fiscal_year']?$_REQUEST['fiscal_year']:date('Y')),
					'date'=>($_REQUEST['xdate']?$_REQUEST['xdate']:date('Y-m-d')),
					'type'=>$_REQUEST['type'],
					'dr'=>preg_replace("/[^0-9.-]/", "", $_REQUEST['dr_amt'][$i]),
					'cr'=>preg_replace("/[^0-9.-]/", "", $_REQUEST['cr_amt'][$i]),
					'ref_id'=>'',
					'payto'=>$_REQUEST['checkinfo'][$_REQUEST['code'][$i]]['payto'],
					'check_date'=>$_REQUEST['checkinfo'][$_REQUEST['code'][$i]]['check_date'],
					'check_number'=>$_REQUEST['checkinfo'][$_REQUEST['code'][$i]]['check_num'],
					'bank'=>$_REQUEST['checkinfo'][$_REQUEST['code'][$i]]['check_bank'],
					'bank_validation'=>$_REQUEST['checkinfo'][$_REQUEST['code'][$i]]['bank_validation'],
					'ar_refid'=>$_REQUEST['ar_refid'][$_REQUEST['code'][$i]],
					'ap_refid'=>$_REQUEST['ap_refid'][$_REQUEST['code'][$i]]
				);
			...
			// $sql = $sql.$sql_data." on duplicate key update fiscal_year=values(fiscal_year),type=values(type),`date`=values(`date`),account_code=values(account_code),
					// account_desc=values(account_desc),dr=values(dr),cr=values(cr),account_group=values(account_group),account_type=values(account_type),report_type=values(report_type),
					// subsidiary=values(subsidiary),cash_in_bank=values(cash_in_bank),
					// payto=values(payto),check_date=values(check_date),check_number=values(check_number),bank=values(bank),bank_validation=values(bank_validation),
					// ar_refid=values(ar_refid),ap_refid=values(ap_refid),center=values(center)";
...
if($_SESSION["vouchering"]['refid']){
	// if($_SESSION["vouchering"]['center']||$_SESSION["vouchering"]['center']!=''){
		// $_SESSION["vouchering"]['center'] = ($_SESSION["vouchering"]['center']=="ucmambalin"?"ucmambaling":$_SESSION["vouchering"]['center']);
		// $info = $db->getWHERE("a.*,b.user certifiedby,c.user approver",
			// "tbl_vouchering_{$_SESSION["vouchering"]['center']} a left join tbl_user b on a.certifiedcorrect=b.id left join tbl_user c on a.approvedby=c.id",
			// "where a.id='{$_SESSION["vouchering"]['refid']}' and a.type='{$_SESSION["vouchering"]['type']}'");
		// $journal = $db->resultArray("*","tbl_journal_entry_{$_SESSION["vouchering"]['center']}","where refid='{$_SESSION["vouchering"]['refid']}' and type='{$_SESSION["vouchering"]['type']}' order by dr desc");
	// }else{
		// $info = $db->getWHERE("a.*,b.user certifiedby,c.user approver",
			// "tbl_vouchering a left join tbl_user b on a.certifiedcorrect=b.id left join tbl_user c on a.approvedby=c.id",
			// "where a.id='{$_SESSION["vouchering"]['refid']}' and a.type='{$_SESSION["vouchering"]['type']}'");
		// $journal = $db->resultArray("*","tbl_journal_entry","where refid='{$_SESSION["vouchering"]['refid']}' and type='{$_SESSION["vouchering"]['type']}' order by dr desc");
	// }
	$info = $db->getWHERE("a.*,b.user certifiedby,c.user approver",
		"tbl_vouchering a left join tbl_user b on a.certifiedcorrect=b.id left join tbl_user c on a.approvedby=c.id",
				...
				<div style="float:left;border:1px solid #000;margin-right:5px;padding:5px;">
					<div style="float:left;margin-right:5px;">
						...
						<div id="payto_area" style="float:left;margin-right:5px;">
							...
							<input style="float:left;width:235px;" type="text" name="fiscal_year" id="fiscal_year" value="<?=date('Y')?>" />
						</div>
						<div style="clear:both;height:5px;"></div>
						<div style="float:left;margin-right:5px;">
		...
		<div class="content" style="min-height:300px;">
			<div style="float:left;width:170px;text-align:center;">
				...
				<fieldset style="padding:5px;text-align:center;">
					...
					<input id="bt9" class="buthov" type="button" value="Print" onclick="viewReport('./reports/vouchering.php?refid=<?=$info['id'] ?>&center=<?=$info['center']?>');" style="height:40px;width:150px;"/>
								...
								if($journal){
									...
									foreach($journal as $key=>$val){
										...
										switch($chart['sub_account']){
											case'CASH':
												$modal = '<fieldset><input type="hidden" name="checkinfo['.$val['account_code'].'][acctcode]" value="'.$val['account_code'].'"/>
														<div style="float:left;margin-right:10px;width:100px;">PayTo</div>
														<input class="check_field" type="text" name="checkinfo['.$val['account_code'].'][payto]" value="'.$val['payto'].'" style="float:left;width:150px;"/>
														<div style="clear:both;height:5px;"></div>
														<div style="float:left;margin-right:10px;width:100px;">Check Date</div>
														<input class="check_field xdate" type="text" name="checkinfo['.$val['account_code'].'][check_date]" value="'.$val['check_date'].'" style="float:left;width:150px;"/>
														<div style="clear:both;height:5px;"></div>
														<div style="float:left;margin-right:10px;width:100px;">Check Number</div>
														<input class="check_field" type="text" name="checkinfo['.$val['account_code'].'][check_num]" value="'.$val['check_number'].'" style="float:left;width:150px;"/>
														<div style="clear:both;height:5px;"></div>
														<div style="float:left;margin-right:10px;width:100px;">Bank</div>
														<input class="check_field" type="text" name="checkinfo['.$val['account_code'].'][check_bank]" value="'.$val['bank'].'" style="float:left;width:150px;"/>
														<div style="clear:both;height:5px;"></div>
														<div style="float:left;margin-right:10px;width:100px;">Bank Validation</div>
														<input class="check_field" type="text" name="checkinfo['.$val['account_code'].'][bank_validation]" value="'.$val['bank_validation'].'" style="float:left;width:150px;"/></fieldset>';
											break;
											case'ACCOUNTS RECEIVABLE':
												...
												$modal = '<div style="float:left;margin-right:10px;width:100px;">Customer Name</div>'.
													'<select name="ar_refid['.$val['account_code'].']" style="float:left;width:250px;">'.
											...
											case'CURRENT LIABILITIES':
												...
												$modal = '<div style="float:left;margin-right:10px;width:100px;">Supplier Name</div>'.
													'<select name="ap_refid['.$val['account_code'].']" style="float:left;width:250px;">'.
											...
											case'OTHER INCOME':
												...
												$modal = '<div style="float:left;margin-right:10px;width:100px;">Cost Center</div>'.
													'<select name="cost_center['.$val['account_code'].']" style="float:left;width:250px;">'.
										...
										echo '<tr id="'.$id.'">
											...
											<td><input onchange="sumDRAmt()"  type="text" class="dr_amt" name="dr_amt[]" style="text-align:right;background:transparent;border:none;width:100px;" value="'.($val['dr']!=0?number_format($val['dr'],2):"").'"/></td>
											<td><input onchange="sumCRAmt()" type="text" class="cr_amt" name="cr_amt[]" style="text-align:right;background:transparent;border:none;width:100px;" value="'.($val['cr']!=0?number_format($val['cr'],2):"").'"/></td>
											<td class="modal" style="display:none;"><div class="modal-content">'.$close.$modal.'</div></td>
											<td style="display:none;"><input type="hidden" name="id[]" value="'.$val['id'].'"/></td>
										</tr>';
...
function getAddInfo(id){
	var code =$('#'+id).find("td input[name='code[]']").val();
	var desc = $('#'+id).find("td input[name='desc[]']").val();
	var icon = '<img onclick="viewAddInfo(\''+id+'\')" src="./images/cashdetails.png" style="float:left;width:20px;height:20px;"/>';
	$.ajax({
		url: './content/vouchering_ajax.php?execute=addInfo&acctcode='+code+'&id='+id+'&desc='+desc,
		type:"POST",
		success:function(data){
			if(data!=''){
				$('#'+id).find("td").eq(0).append(icon);
				$('#'+id).find("td").eq(5).html(data);
			}
		}
	});
}
