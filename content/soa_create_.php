...
if($_REQUEST['gensoa']){ //Generate New SOA
	...
	if(date('Ym',strtotime($info['contract_end_date']))>date('Ym',mktime(0,0,0,$_REQUEST['month'],1,date('Y')))){
		$totalrent_amt = $info['fixed_rental_amount'];
		...
		$oldrent_perday = $info['fixed_rental_amount']/$numDays;
		$oldrent_days = date('d',strtotime($info['contract_end_date']));
		$newrent_perday = ($info['fixed_rental_amount']*1.05)/$numDays;
...
function sumVoucherAmt(){
	...
	console.log('wtax:'+(strtodouble(wtax)));
...
function sumRCAmt(){
	...
	//return sum;
	console.log(sum+'\n');
	//return new Number().formatMoney(2);
	return sum;
}
