...
if($_REQUEST['execute']){
				...
				<tbody style="font-size:10px;">
					<?php foreach($list as $key => $val){ ?>
						<tr>
							...
							<td style="text-align:center;"><a href="./reports/dynamic_invoicing<?=$_SESSION['repExtension']?>.php?refid=<?=$val['id']?>&tbltype=<?=$_REQUEST['tbltype']?>" target="_blank"><img src="./images/print.png" style="width:25px;height:25px;" title="Print"/></a></td>
						</tr>
					<?php } ?>
				</tbody>
			</table>
	...
	if($_POST){
		...
		if($qry1){
			...
			$flag=false;
			$sql2="insert into tbl_{$_REQUEST['tbltype']}_items (id,refid,skuid,barcode,divmul,item_spec,qty,unit,unitprice,amount,cost) values ";
			$xtotal=0;
			$cost_total=0;
			foreach($_REQUEST['items'] as $key=>$val){
				if($flag)$sql2.=",";
				$sql2.="('{$val['id']}','{$refid}','{$val['skuid']}','{$val['barcode']}','{$val['divmul']}','".mysql_real_escape_string($val['items_spec'])."','{$val['qty']}','".mysql_real_escape_string($val['unit'])."','".preg_replace("/[^0-9.-]/", "", $val['unit_price'])."','".preg_replace("/[^0-9.-]/", "", $val['amount'])."','".preg_replace("/[^0-9.-]/", "", $val['cost'])."')";
				$flag=true;
	...
	<div style="clear:both;height:10px;"></div>
		<div style="width:25%;float:left;">
			<fieldset>
				...
				<input value="<?=$info['customer_address']?>" type="text" style="width:250px;"/>
				<div style="clear:both;height:5px;"></div>
				<div style="width:150px;float:left;">Telephone Number</div>
				<input value="<?=$info['contact_number']?>" type="text" style="width:250px;"/>
				<div style="clear:both;height:5px;"></div>
				<div style="width:150px;float:left;">Fax Number</div>
				<input value="<?=$info['fax_person']?>" type="text" style="width:250px;"/>
				<div style="clear:both;height:5px;"></div>
				<div style="width:150px;float:left;">Contact Person</div>
				<input value="<?=$header['attn']?$header['attn']:$info['contact_person']?>" type="text" name="info[attn]" style="width:250px;"/>
				<div style="clear:both;height:5px;"></div>
				<div style="width:150px;float:left;">TIN</div>
				<input value="<?=$info['tin']?>" type="text" style="width:250px;"/>
				<div style="clear:both;height:5px;"></div>
			</fieldset>
			<fieldset>
				<legend>Details</legend>
					...
					<input value="<?=$header['date']?$header['date']:date('Y-m-d')?>" type="text" name="info[date]" style="width:150px;"/>
					<div style="clear:both;height:5px;"></div>
					<div style="width:150px;float:left;">Payment Terms</div>
					<select name="info[payment_terms]" style="width:155px;">
						<option value="">Select</option>
						<?php
						$terms = array("COD","15 DAYS","30 DAYS","60 DAYS");
						foreach($terms as $key => $val){
							echo "<option ".($header['payment_terms']==$val?"selected":"")." value='{$val}'>{$val}</option>";
						}
						?>
					</select>
					<div style="clear:both;height:5px;"></div>
					...
					<div style="clear:both;height:5px;"></div>
					<div style="width:150px;float:left;">REGION:</div>
					<input value="<?=$header['region']?>" type="text" name="info[region]" style="width:150px;"/>
					<div style="clear:both;height:5px;"></div>
					<div style="width:150px;float:left;">REP:</div>
					<select name="info[rep]" style="width:155px;">
						<option value="">Select</option>
						<?php
						$rep = $db->resultArray("*","req_agent","");
						foreach($rep as $key => $val){
							echo "<option ".($header['rep']==$val['id']?"selected":"")." value='{$val['id']}'>{$val['agent_name']}</option>";
						}
						?>
					</select>
					<div style="clear:both;height:5px;"></div>
...
function getStockOut(){
	...
	$.ajax({
		url: './content/tkc/dynamic_invoicing.php?execute=getStockOut&stockoutref='+ref,
		type:"POST",
		dataType: 'json',
...
function toSession(){
	...
	if(custid!=""){
		$.ajax({
			url: './content/tkc/dynamic_invoicing.php?execute=toSession&custid='+custid,
			type:"POST",
			success:function(data){
...
function createNew(){
	$.ajax({
		url: './content/tkc/dynamic_invoicing.php?execute=createNew',
		type:"POST",
		success:function(data){
			window.location=removeURLParameter(document.URL, 'refid');
		}
...
function viewlist(){
	var custid = $("#business_name").val();
	clickDialogUrl("dialogbox3",900,500,'./content/tkc/dynamic_invoicing.php?tbltype='+getParam('tbltype')+'&execute=list&custid='+custid,"Listing");
}
function search(){
	var search = $("#search").val();
	var date = $("input[name='date']").val();
	clickDialogUrl("dialogbox3",900,500,'./content/tkc/dynamic_invoicing.php?tbltype='+getParam('tbltype')+'&execute=list&search='+search+'&date='+date,"Listing");
}
$("input[value='Prod Search']").on('click',function(){
	clickDialogUrl("prodlist",1000,550,'./content/pos_ajax.php?execute=prodlist',"Product Search");
});
...
function addItemDetails(barcode){
	var num =$("#items_tbl tbody tr").length;
	$.ajax({
		url: './content/tkc/dynamic_invoicing.php?execute=selectedProd&barcode='+barcode,
		type:"POST",
		dataType: 'json',
		success:function(data){
			$.each(data.rec, function(k, v) {
