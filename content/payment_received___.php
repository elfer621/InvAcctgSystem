...
if($_POST){
	...
	$receipt="insert into tbl_receipt_manual (`receipt`,`date`,`cust_id`,`receivedfrom`,`paymentof`,
		`cash_amt`,`check_amt`,`check_details`,`check_date`,`particular`,rent_amount,other_amount,total_amount,bp_date,bp_bank,bp_amt,inwords) values 
		...
		'{$_REQUEST['CRJ']['bp_date']}','{$_REQUEST['CRJ']['bp_bank']}','{$_REQUEST['CRJ']['bp_amt']}','".ucwords($db->intToWords((double)preg_replace("/[^0-9.]/", "", $_REQUEST['xtotal'])))."')";
		...
		<table class="navigateableMain" id="mytbl" cellspacing="0" cellpadding="0" width="100%">
			<thead>
				...
				<tr>
					...
					<td style="border:1px solid #1DA5EA;">
						<input type="text" id="rent_amount" name="rent_amount" style="text-align:right;background:transparent;border:none;width:100%;border:1px solid red;"/>
					</td>
				</tr>
				<tr>
...
$("#rent_amount,#other_amount").change(function(){
	$(this).val(new Number($(this).val()).formatMoney(2));
	calculate();
});
