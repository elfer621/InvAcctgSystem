	...
	<table id="mytbl" class="print tbl" cellspacing="0" cellpadding="0" width="100%">
		...
		<tbody>
			...
			<tr style="<?=$row['type']=="VOID"?"color:red;":""?>">
				...
				<td id="seq_<?=$row['receipt']?>_<?=$row['reading']?>_<?=$row['counter']?>" style="text-align:center;"><?= $row['xstudentid']." ".$row['studentname']?></td>
				<td style="text-align:center;"><?= $row['course']." ".$row['yr']?></td>
...
function viewReceipt(num,readingnum,counter) {
	...
	var page = './receipt_forcommission.php?receipt_num='+num+'&readingnum='+readingnum+'&counter='+counter+'&reprint=true&computecommission=true';
