...
<div style="width:300px;font-family:Calibri,Arial, Verdana, Geneva, Helvetica, Sans-Serif;">
	...
	foreach($all as $k=>$v){
		...
		$persup=$db->resultArray("b.supplier_id,sum(qty*selling) as total_selling,sum(qty*cost) as total_cost","tbl_sales_items a left join tbl_product_name b on a.skuid=b.sku_id","where a.reading='$reading' and a.counter='$counter' and a.category_id='1' group by b.supplier_id");
		$sql = "SELECT 
				...
				(sum(book_sales)+sum(idsling_sales)) - sum(voucher_amt) cash_amt,
	...
	if(!$_REQUEST['all'] && date('Y-m-d',strtotime($info['start_date']))>='2017-03-01'){ //journal entry...
		...
		// print_r($entry);
		// echo "</pre>";
		$glid=$con->insertSJDiffApproach($glref,$sql,$info_r['start_date'],$entry,'SJ');
