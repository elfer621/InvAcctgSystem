 ...
 */
jQuery.tableNavigationMain = function(settings) {
	...
	if (jQuery.extendedNavigationMain.settings == null) {
		jQuery.extendedNavigationMain.settings = jQuery.extend({
			table_selector: 'table.navigateableMain',
			row_selector: 'table.navigateableMain tbody tr',
			selected_class: 'selected',
			activation_selector: 'a.activationMain',
		...
		jQuery.extendedNavigationMain.settings = jQuery.extend(jQuery.extendedNavigationMain.settings, settings);
		jQuery.extendedNavigationMain.tables = jQuery(jQuery.extendedNavigationMain.settings.table_selector);
	 ...
	 * jQuery.extendedNavigationMain.internal_event_hanlders object. This makes it possible to reuse the
	...
	function defineInternalEventHandlers() {
		jQuery.extendedNavigationMain.internal_event_hanlders.click = function(){
			if ( jQuery.extendedNavigationMain.settings.disabled ) { return }
			selectRow(this);
		};
		
		jQuery.extendedNavigationMain.internal_event_hanlders.dblclick = function(){
			if ( jQuery.extendedNavigationMain.settings.disabled ) { return }
			activateRow(this);
		};
		
		jQuery.extendedNavigationMain.internal_event_hanlders.activation_link_click = function(){
			if ( jQuery.extendedNavigationMain.settings.disabled ) { return }
			activateRow(jQuery(this).parents('tr').get(0));
			return false;
		};
		
		jQuery.extendedNavigationMain.internal_event_hanlders.activation_link_focus = function(){
			if ( jQuery.extendedNavigationMain.settings.disabled ) { return }
			selectRow(jQuery(this).parents('tr').get(0));
		};
		
		jQuery.extendedNavigationMain.internal_event_hanlders.keypress = function(e){
			//e.preventDefault();
			if ( jQuery.extendedNavigationMain.settings.disabled ) { return }
			...
			switch (key_code){
				...
				case 39: // arrow right
					...
					if (next_row.length > 0) {
						...
						return r;
					} else if ( jQuery.extendedNavigationMain.settings.jump_between_tables ) {
						var current_table = getSelectedRow().parents(jQuery.extendedNavigationMain.settings.table_selector).get(0);
						var all_tables = jQuery.extendedNavigationMain.tables;
	...
	function bindInternalEventHandlers() {
		var selectable_rows = jQuery(jQuery.extendedNavigationMain.settings.row_selector);
		var selected_row = selectable_rows.filter("." + jQuery.extendedNavigationMain.settings.selected_class);
		...
		selectable_rows.bind(jQuery.extendedNavigationMain.settings.select_event, {}, jQuery.extendedNavigationMain.internal_event_hanlders.click);
		selectable_rows.bind(jQuery.extendedNavigationMain.settings.activate_event, {}, jQuery.extendedNavigationMain.internal_event_hanlders.dblclick);
		
		var activation_element = selectable_rows.find(jQuery.extendedNavigationMain.settings.activation_selector);
		if ( activation_element && jQuery.extendedNavigationMain.settings.activation_element_activate_event ) {
			selectable_rows.find(jQuery.extendedNavigationMain.settings.activation_selector).bind(jQuery.extendedNavigationMain.settings.activation_element_activate_event, {}, jQuery.extendedNavigationMain.internal_event_hanlders.activation_link_click);
		}
		activation_element.focus(jQuery.extendedNavigationMain.internal_event_hanlders.activation_link_focus);
		
		if (jQuery.extendedNavigationMain.settings.bind_key_events_to_links) {
			var bind_to = jQuery.extendedNavigationMain.settings.row_selector + ' ' + jQuery.extendedNavigationMain.settings.activation_selector;
		} else {
		...
		if (jQuery.browser.msie) {
			jQuery(bind_to).keydown(jQuery.extendedNavigationMain.internal_event_hanlders.keypress);
		} else {
			jQuery(bind_to).keypress(jQuery.extendedNavigationMain.internal_event_hanlders.keypress);
		}
	};
	 ...
	 * executed. See "jQuery.extendedNavigationMain.settings.on_select" configuration option.
	...
	function selectRow(row) {
		if (row != undefined && row.nodeName.toLowerCase() == "tr"){
			var do_default_action = executeEventHandler(row, jQuery.extendedNavigationMain.settings.on_select);
			
			if (do_default_action)
			{
				getSelectedRow().removeClass(jQuery.extendedNavigationMain.settings.selected_class);
				jQuery(row).addClass(jQuery.extendedNavigationMain.settings.selected_class);
				
				if (jQuery.extendedNavigationMain.settings.focus_links_on_select) {
					var activation_element = jQuery(row).find(jQuery.extendedNavigationMain.settings.activation_selector)[0];
					if (activation_element != null) {
						activation_element.focus();
					}
				}
				
				if (jQuery.extendedNavigationMain.settings.focus_tables) {
					jQuery.extendedNavigationMain.tables.removeClass(jQuery.extendedNavigationMain.settings.focused_table_class);
					jQuery(row).parents(jQuery.extendedNavigationMain.settings.table_selector).eq(0).addClass(jQuery.extendedNavigationMain.settings.focused_table_class);
				}
	...
	function getSelectedRow() {
		return jQuery(jQuery.extendedNavigationMain.settings.row_selector + "." + jQuery.extendedNavigationMain.settings.selected_class);
	}
	 ...
	 *   "jQuery.extendedNavigationMain.settings.cookie_name" is not null.
	 * - If present the user specified on_activate event handler will be
	 *   executed. See "jQuery.extendedNavigationMain.settings.on_activate" or the
	 ...
	 *   element selected by the "jQuery.extendedNavigationMain.settings.activation_selector"
	...
	function activateRow(row) {
		if (row != undefined) {
			...
			var activated_index = jQuery(jQuery.extendedNavigationMain.settings.row_selector).index(row);
			storeLastRowIndex(activated_index);
			
			var do_default_action = executeEventHandler(row, jQuery.extendedNavigationMain.settings.on_activate);
			
			if (do_default_action)
			{
				var activation_link = jQuery(jQuery.extendedNavigationMain.settings.activation_selector, row);
	...
	function executeEventHandler(row, action) {
		...
		{
			try {
				var onclick_of_target_link = jQuery(jQuery.extendedNavigationMain.settings.activation_selector, row).attr(action);
	...
	function scrollToIfNeccessary(e) {
		...
		var calculated_boundaries = {
			top: element_position.y - jQuery.extendedNavigationMain.settings.scroll_overlap,
			bottom: element_position.y + element_position.hb + jQuery.extendedNavigationMain.settings.scroll_overlap,
			left: element_position.x - jQuery.extendedNavigationMain.settings.scroll_overlap,
			right: element_position.x + element_position.wb + jQuery.extendedNavigationMain.settings.scroll_overlap
		};
	...
	function readLastRowIndex() {
		if (jQuery.extendedNavigationMain.settings.cookie_name)
		{
			var data = readCookie(jQuery.extendedNavigationMain.settings.cookie_name);
			if (data) {
	...
	function storeLastRowIndex(index) {
		if (jQuery.extendedNavigationMain.settings.cookie_name)
		{
			...
			createCookie(jQuery.extendedNavigationMain.settings.cookie_name, data);
 		}
	};
	...
	function eraseLastRowIndexCookie() {
		eraseCookie(jQuery.extendedNavigationMain.settings.cookie_name);
	};
